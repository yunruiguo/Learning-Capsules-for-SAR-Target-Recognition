...
def routing(u_hat, c_ij, nb_capsules, nb_capsules_p):
    """
        Routing algorithm
...
def fully_connected_caps_layer(input_layer, capsules_size, nb_capsules):
    """
    ...
    b_ij = tf.get_variable('b_ij', shape=[nb_capsules_p, nb_capsules, 1, 1], dtype=tf.float32, initializer=tf.constant_initializer(1.0))
    return routing(u_hat, b_ij, nb_capsules, nb_capsules_p)
